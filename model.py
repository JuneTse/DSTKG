...
class DBTKGE(nn.Module):
    def __init__(self, kg, embedding_dim, batch_size, learning_rate, L, gran, gamma, n_day, gpu=True):
        ...
        self.ent_mean = nn.Embedding(self.kg.n_entity, self.embedding_dim,_weight=torch.zeros(self.kg.n_entity, self.embedding_dim)) #ones ??
        self.ent_std = nn.Embedding(self.kg.n_entity, self.embedding_dim,_weight=torch.ones(self.kg.n_entity, self.embedding_dim))
    ...
    def forward_kl_loss(self, pos):
        ...
        return (trans_loss + kl_s + score_s )*self.kl_parameter
