...
class AVB(nn.Module):
    def __init__(self, kg, embedding_dim, batch_size, learning_rate, L, gran, gamma, n_day, gpu=True):
        ...
        self.time_ent = nn.Linear(self.embedding_dim * 2, self.embedding_dim)

        self.sd_h = nn.Linear(self.embedding_dim * 2, self.embedding_dim)
        self.sd_t = nn.Linear(self.embedding_dim * 2, self.embedding_dim)

        self.ent = nn.Embedding(self.kg.n_entity, self.embedding_dim)
    ...
    def get_embedding(self,X,i=0):
        ...
        h = self.ent(h_i)
        h = self.time_ent(torch.cat([h, pri_time_laten], 1))
        t =  self.ent(t_i)
        t = self.time_ent(torch.cat([t, pri_time_laten], 1))
        r = self.rel_embedding(r_i)
        return h,r,t
    def forward(self, X,i = 0):
        h_real,r,t_real =self.get_embedding(X,i)
        # z_h, _ = self.gru(self.embed_dropout(h_real))
        # z_t, _ = self.gru(self.embed_dropout(t_real))
        # 潜在变量
        z_h = self.eps(self.embed_dropout(h_real))
        z_t = self.eps(self.embed_dropout(t_real))
        # 解码器
        h = self.linear1(z_h)
        h = self.activation(h)
        h = self.linear2(h)
