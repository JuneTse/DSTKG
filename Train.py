...
from Dataset_YG1 import KnowledgeGraphYG1
...
def train(task ='LinkPrediction',
    ...
    if data_dir == 'yago' or data_dir == 'wikidata':
        ...
        n_day = kg.n_time
        print("num_time:",n_day,"num_ent:",kg.n_entity,"num_rel:",kg.n_relation)
        min_epoch=10
    if data_dir == 'YAGO1830':
        kg = KnowledgeGraphYG1(data_dir=data_dir, count = count,rev_set = rev_set)
        n_day = kg.n_time
        print("num_time:",n_day,"num_ent:",kg.n_entity,"num_rel:",kg.n_relation)
        min_epoch=10
    ...
    if modelname== 'DBTKGE':
        model = KGE.DBTKGE(kg, embedding_dim=dim, batch_size=batch, learning_rate=lr, gamma=gamma, L=L, gran=gran, n_day=kg.n_time, gpu=cuda_able)
    if modelname=='ATISE':
    ...
    for epoch in range(max_epoch):
        ...
        for iter_triple in train_triple:
            # print(max(iter_triple[:, 0]), max(iter_triple[:, 1]), max(iter_triple[:, 2]), max(iter_triple[:, 3]),
            #       max(iter_triple[:, 4]), )
            ...
            pos_score = model.forward(iter_triple,0)
            neg_score = model.forward(iter_neg,1)
            if timedisc ==2:
                pos_score += model.forward(iter_triple_e,0)
                neg_score += model.forward(iter_neg_e,1)
            ...
            trans_loss=model.forward_kl_loss(iter_triple)
            # s = neg - pos
            loss=loss+trans_loss
        ...
        if (epoch+1)//min_epoch>epoch//min_epoch and epoch < max_epoch:
            ...
            if mean_rr < mrr_std and patience<3:
                patience+=1
            if (mean_rr < mrr_std and patience>=3) or epoch==max_epoch-1:
